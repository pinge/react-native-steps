...
export async function canCountSteps() {
...
export async function startCounting(options) {
  const {
    since,
    onStep,
    onError,
  ...
  return ReactNativeSteps.start(resolvedSince, notification, cadence, goal).catch(error => {
    clearSubscriptions();
    throw error;
  });
}
...
/** Stop counting steps. */
export async function stopCounting(options) {
  const {
    clear = false
  } = options ?? {};
  clearSubscriptions();
 ...
 */
export async function isCounting() {
